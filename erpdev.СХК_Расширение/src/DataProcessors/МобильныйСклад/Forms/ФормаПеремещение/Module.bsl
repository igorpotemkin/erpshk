...
Функция ПереместитьНаСервере() 
	...
	Иначе
		...
		Если ЭтаФорма.ЯчейкаОтправитель <> Справочники.СХК_СкладЯчейки.ПустаяСсылка() И ЭтаФорма.ЯчейкаПолучатель <> Справочники.СХК_СкладЯчейки.ПустаяСсылка() Тогда
			Если ЭтаФорма.Количество>0  
				И ЭтаФорма.Количество <= СХК_ТСДСклад.ОстатокЯчейкаНоменклатура(ЭтаФорма.ЯчейкаОтправитель,ЭтаФорма.Номенклатура,ЭтаФорма.Серия.Наименование ,ТекДок.Дата ) Тогда
					//КолЯчейка = СХК_ТСДСклад.СодержимоеЯчейки(ЭтаФорма.ЯчейкаПолучатель); 
					//
					//Если КолЯчейка>0 Тогда
					//	Если ЭтаФорма.ЯчейкаПолучатель.ВМестимость = Перечисления.СХК_ВМестимость.Палет Тогда
					//		Если КолЯчейка + ЭтаФорма.Количество > 20000 Тогда
					//			Ош = "В ячейке доступно места" + Строка(20000-КолЯчейка);
					//		Иначе
					//			Ош = Истина;
					//		КонецЕсли; 
					//	ИначеЕсли ЭтаФорма.ЯчейкаПолучатель.ВМестимость = Перечисления.СХК_ВМестимость.Сборный Тогда
					//		Если КолЯчейка + ЭтаФорма.Количество > 300000 Тогда
					//			Ош = "В ячейке доступно места" + Строка(300000-КолЯчейка);
					//		Иначе
					//			Ош = Истина;
					//		КонецЕсли; 
					//	Иначе
					...
					//Если ЭтаФорма.ЯчейкаПолучатель.ВМестимость = Перечисления.СХК_ВМестимость.Палет Тогда
						//Если СХК_ТСДСклад.СодержимоеЯчейки(ЭтаФорма.ЯчейкаПолучатель)-ЭтаФорма.Количество 							 

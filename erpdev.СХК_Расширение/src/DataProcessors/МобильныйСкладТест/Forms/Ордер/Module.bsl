...
Процедура СписокТоварыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка) 
	...
	Если ТИПЗНЧ(НСтр) = ТИП("Число") Тогда
		...
		ОткрытьФорму("Обработка.МобильныйСкладТест.Форма.ФормаСборки", _Параметры,ЭтаФорма, , , , ОписаниеОповещения,РежимОткрытияОкнаФормы.Независимый);
	Иначе
...
//	КонецЕсли;
//	
//СписокТовары.Параметры.УстановитьЗначениеПараметра("Ссылка",ЭтаФорма.СХКСписание);	
//		
//	
//	
//	//Если СтруктураПараметров.Ссылка <> "" Тогда
...
//	//			Т = Док.ТоварыПоРаспоряжениям.Добавить();
...
//	Отбор = Новый Структура;
//	Отбор.Вставить("Серия",Сер);
//	Строки = Д.ТоварыПоРаспоряжениям.НайтиСтроки(Отбор);
//	Д.ТоварыПоРаспоряжениям.Удалить(Строки[0].НомерСтроки-1);
//	
//	Для Каждого строка из ТЗ Цикл
//		Т = Д.ТоварыПоРаспоряжениям.Добавить();
...
//	КонецЦикла;  
//	Д.ТоварыПоРаспоряжениям.Свернуть("Номенклатура,Серия,НЯчейка","Количество");
//	Д.Записать(РежимЗаписиДокумента.Проведение);
//	

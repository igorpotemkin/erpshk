...
//15    <EAN13 AddOn 5>
//16    <QR>
//17    <GS1 databar extended stacked>
//18    <DataMatrix ASCII>
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПодборНоменклатуры.Параметры.УстановитьЗначениеПараметра("Родитель", Справочники.Номенклатура.НайтиПоКоду("00000000007")); 
...
Функция ПечатьНаСервере(ТаблДокум)
	...
	Для Каждого стр из ТЧТовары Цикл
		...
		QRСтрока = Строка(стр.Серия)+";"+Строка(стр.Штрихкод);
		...
		//	Срок = 12;
		//КонецЕсли;
		Срок = стр.Серия.СХК_СрокГодности;
		
		//ОбластьСтрока.Параметры.Срок = "Срок годности: "+стр.Номенклатура.СрокГодности+" месяцев";
		ОбластьСтрока.Параметры.Срок = "Срок годности: "+Срок+" месяцев";
		...
		ПараметрыШтрихкода.Вставить("ТипКода",         Число(ЭтаФорма.ТипКода)); // EAN13
		ПараметрыШтрихкода.Вставить("ОтображатьТекст", Истина);
		ПараметрыШтрихкода.Вставить("РазмерШрифта",    56);
		
		ОбластьПодвал.Рисунки.ШтрихКод.Картинка= ПолучитьКартинкуШтрихкода(ПараметрыШтрихкода);
...
Функция ПечатьНаСервереШоуПринт(ТаблДокум)
	...
	Для Каждого стр из ТЧТовары Цикл
		...
		ОбластьСтрока.Параметры.Текст1 =стр.УсловияХранения;
		ОбластьСтрока.Параметры.Текст2 =ТекстЭтикетки;  
		Если стр.ВариантУпаковки.Числитель = 0 Тогда
			ОбластьСтрока.Параметры.Масса = "Масса нетто: " + Строка(стр.Нетто) + " г. (" +Строка(стр.КолШт) + " шт по "+ Строка(стр.Нетто/Число(стр.КолШт)) + " г. )";
		Иначе
			ОбластьСтрока.Параметры.Масса = "Масса нетто: " + Строка(стр.Нетто) + " г. (" +Строка(стр.ВариантУпаковки.Числитель) + " шт по "+ Строка(стр.весшт) + " г. )";
		КонецЕсли;
...
Функция ПечатьНаСервереАнг(ТаблДокум)
	...
	Для Каждого стр из ТЧТовары Цикл
		...
		ОбластьСтрока.Параметры.Срок = "Shelf life: "+Срок+" MONTHS.";
		ОбластьСтрока.Параметры.Телефон = "+7 (499) 322-48-37";
		Если стр.СерияЭтикетки = "MUSLER" ИЛИ стр.СерияЭтикетки = "MUSLER PRO" 
			ИЛИ стр.СерияЭтикетки = "ОРЕХОВЫЕ" 
			ИЛИ стр.СерияЭтикетки = "ПОЧТОВАЯ МАРКА" Тогда 
...
Процедура СЗЗаданиеОбработкаВыбораНаСервере(ДЗПроизв,ДЗНоменклатура,ДЗПериод,ДЗСерия)
	...
	Если ТабЗнач.Количество()>=1 Тогда 
		...
		Запрос.УстановитьПараметр("Номенклатура", Спр.Ссылка);
		...
		Если РезультатЗапроса.Следующий() Тогда
			НовСтрока.Штрихкод = РезультатЗапроса.Штрихкод;
		Иначе
			КолВоШт = Спр.КоэффициентЕдиницыДляОтчетов;  
			Если Спр.ИспользоватьУпаковки И КолВоШт > 0 Тогда
				КолВоШтВКороб   = КолВоШт;
				ЕдИзмУпаковки   = Справочники.УпаковкиЕдиницыИзмерения.ПустаяСсылка();
				ПоискПоУпаковке = Ложь;
				
				Если ЗначениеЗаполнено(Упаковка) Тогда
					КолВоШтВКороб   = КолВоШт * ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Упаковка, "Числитель");
					ЕдИзмУпаковки   = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Упаковка, "ЕдиницаИзмерения");
					ПоискПоУпаковке = Истина;
				КонецЕсли;
				...
				Запрос.Текст = "ВЫБРАТЬ
							   ...
							   |			И ШтрихкодыНоменклатуры.Упаковка.Числитель = &Числитель
							   |			И ШтрихкодыНоменклатуры.Упаковка.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.Упаковка)";
				
				Если ПоискПоУпаковке Тогда
					Запрос.Текст = Запрос.Текст + "
					|			И ШтрихкодыНоменклатуры.Упаковка.ЕдиницаИзмерения = &ЕдиницаИзмерения";
					
					Запрос.УстановитьПараметр("ЕдиницаИзмерения", ЕдИзмУпаковки);
				КонецЕсли;
				
				Запрос.УстановитьПараметр("Номенклатура", Спр.Ссылка);
				Запрос.УстановитьПараметр("Числитель", КолВоШтВКороб);
...
| И ШтрихкодыНоменклатуры.Упаковка.Числитель = &Числитель";
						...
						Если ВыборкаДетальныеЗаписи.Следующий() Тогда   
							 НовСтрока.Штрихкод = ВыборкаДетальныеЗаписи.Штрихкод;
						 КонецЕсли;  
						 
						
						 Если Не ЗначениеЗаполнено(НовСтрока.Штрихкод) тогда
...
//|							   			И ШтрихкодыНоменклатуры.Упаковка.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.Упаковка)";
//						
//						Запрос.УстановитьПараметр("Номенклатура", ТабЗнач[0].Номенклатура);
//						
//						РезультатЗапроса = Запрос.Выполнить();
//						
//						ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
...
Процедура ОтменитьЗадание(Команда) 
	...
	&НаКлиенте
	Процедура ПечатьБеларусскийМаркиратор(Команда)
		Ош = ПроверкаПолей();
		Если Ош = Ложь Тогда
			ТаблДокум = Новый  ТабличныйДокумент;
			ТаблДокум = ПечатьНаСервереБелМаркератор(ТаблДокум);
			ТаблДокум.Показать("Печать документа");
		Иначе
			ДиалогОшибки(Ош);
		КонецЕсли;
...
&НаСервере
Функция ПечатьНаСервереБелМаркератор(ТаблДокум)  
	Обработка = РеквизитФормыВЗначение("Объект"); 
	ТаблДокум.Очистить();
	Для Каждого стр из ТЧТовары Цикл
		ТипМакет = РеквизитФормыВЗначение("Объект").ПолучитьМакет("МакетШоуБел");	
		...
		ОбластьСтрока.Параметры.Текст4 = "Произведен и упакован по заказу" + символы.пс+ " ООО 'ЕВРОТОРГ'";
		ТаблДокум.Вывести(ОбластьСтрока);
		
		///
		
		ОбластьПодвал = ТипМакет.ПолучитьОбласть("Подвал");
		ОбластьПодвал.Параметры.Текст3 = "Поставщик в Республике Беларусь:"+символы.пс+
										"ООО ""ЕВРОТОРГ"", Республика Беларусь"+символы.пс+
										 "220099, г.Минск, ул.Казинца, дом 52а"+символы.пс+
										"тел.: +375 44 7888 880";
...
&НаКлиенте
Процедура ПечатьНаборы(Команда)
		Ош = ПроверкаПолей();
	Если Ош = Ложь Тогда
		ТаблДокум = Новый  ТабличныйДокумент;
		ТаблДокум = ПечатьНаСервереНаборы(ТаблДокум);
		ТаблДокум.Показать("Печать документа");
	Иначе
		ДиалогОшибки(Ош);
	КонецЕсли;
...
&НаСервере
Функция ПечатьНаСервереНаборы(ТаблДокум)
	Обработка = РеквизитФормыВЗначение("Объект"); 
	
	ТаблДокум.Очистить();
	Для Каждого стр из ТЧТовары Цикл
		ТипМакет = РеквизитФормыВЗначение("Объект").ПолучитьМакет("МакетНаборы");	
		...
		ДопСвойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("Имя", "СоставПродукта_c572cac245af42bf9b0cca89d384272d"); // Получение дополнительного реквизита 
		Значение = УправлениеСвойствами.ЗначениеСвойства(стр.Номенклатура, ДопСвойство);
	//       
		...
		Если Не ПустаяСтрока(QRСтрока) Тогда
			...
			Если ТипЗнч(ДанныеQRКода) = Тип("ДвоичныеДанные") Тогда
				...
				//ОбластьСтрока.Рисунки["QRКод"].Картинка = КартинкаQRКода;
				ОбластьСтрока.Рисунки["QRКод"].Картинка = КартинкаQRКода; 
				ОбластьСтрока.Рисунки["QRКод"].Высота = 5; 
				ОбластьСтрока.Рисунки["QRКод"].Ширина = 20; 
				ОбластьСтрока.Рисунки["QRКод"].Верх =0.1;
			Иначе
				ТекстСообщения = "Не удалось сформировать QR-код для документа";
				#Если Клиент Тогда
					Сообщить(ТекстСообщения);
		...
		КонецЕсли;
		ОбластьСтрока.Параметры.Текст3 =СокрЛП(Значение);
		ОбластьСтрока.Параметры.Текст1 =стр.УсловияХранения;
		ОбластьСтрока.Параметры.Текст2 =ТекстЭтикетки;
		Если стр.ВариантУпаковки.Числитель = 0 Тогда
			ОбластьСтрока.Параметры.Масса = "Масса нетто: " + Строка(стр.Нетто) + " г. (" +Строка(стр.КолШт) + " шт по "+ Строка(стр.Нетто/Число(стр.КолШт)) + " г. )";
...
&НаКлиенте
Процедура Этикетка110х60(Команда)
	Ош = ПроверкаПолей();
	Если Ош = Ложь Тогда
		ТаблДокум = Новый  ТабличныйДокумент;
		ТаблДокум = Этикетка110х60НаСервере(ТаблДокум);
		ТаблДокум.Показать("Печать документа");
	Иначе
		ДиалогОшибки(Ош);
	КонецЕсли;
...
&НаСервере
Функция Этикетка110х60НаСервере(ТаблДокум) 
	Обработка = РеквизитФормыВЗначение("Объект"); 
	
	ТаблДокум.Очистить();
	Для Каждого стр из ТЧТовары Цикл
		ТипМакет = РеквизитФормыВЗначение("Объект").ПолучитьМакет("МакетШоу110х60");	
	...
	ОбластьШапка.Параметры.Наименование	 = Строка(стр.Наименование)+", " + Строка(стр.Номенклатура.Марка);  
		...
		ТаблДокум.Автомасштаб = Истина;  
ТаблДокум.РазмерСтраницы="A4";//нестандартный размер бумаги
ТаблДокум.ВысотаСтраницы=60;
ТаблДокум.ШиринаСтраницы=110;		
	КонецЦикла;
...
&НаКлиенте
	Процедура ТЧТоварыСерияНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
		
		СтандартнаяОбработка = Ложь;
	
	ВидНомен = ПолучитьВидНоменклатуры(ЭтаФорма.Элементы.ТЧТовары.ТекущиеДанные.Номенклатура);	
	
	ЭлементыОтбора = Новый Структура("ВидНоменклатуры,Номенклатура", ВидНомен,ЭтаФорма.Элементы.ТЧТовары.ТекущиеДанные.Номенклатура);
...
Процедура ОбработатьЗакрытиеФормыВыборСерииИзСписка(Значение, ДопПараметры) Экспорт  
	...
	Если ЗначениеЗаполнено(Значение) Тогда    
		нСерия = ПолучитьСериюПоНомеру(Значение.Номер,ЭтаФорма.Элементы.ТЧТовары.ТекущиеДанные.Номенклатура);  
		Если нСерия.Количество()>=1 Тогда
			ЭтаФорма.Элементы.ТЧТовары.ТекущиеДанные.Серия = нСерия[0].Ссылка;	
			ЭтаФорма.Элементы.ТЧТовары.ТекущиеДанные.Срок = нСерия[0].ГоденДо;	
		КонецЕсли;
...
КонецПроцедуры     
 ...
 Функция  ПолучитьСериюПоНомеру(Номер,Номенклатура) 
	 ...
	 Сз.Вставить("Ссылка",Справочники.СерииНоменклатуры.ПустаяСсылка());
	...
	Если Рез.Следующий() Тогда 
		Сз.Вставить("Ссылка",Рез.Ссылка);
		Сз.Вставить("ГоденДо",Рез.Ссылка.СХК_СрокГодности);
		Мас.Добавить(СЗ);
	КонецЕсли;

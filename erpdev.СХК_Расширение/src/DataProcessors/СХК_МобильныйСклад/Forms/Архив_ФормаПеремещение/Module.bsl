...
Функция ПереместитьНаСервере() 
	...
	Иначе
		...
		КонецЕсли;
		Если ЭтаФорма.АдресОткудаЗнач <> Справочники.СХК_СкладЯчейки.ПустаяСсылка() И ЭтаФорма.Ячейка <> Справочники.СХК_СкладЯчейки.ПустаяСсылка() Тогда
					
			Если ЭтаФорма.Количество>0 И 
						ЭтаФорма.Количество <= СХК_ТСДСклад.ОстатокЯчейкаНоменклатура(ЭтаФорма.АдресОткудаЗнач,ЭтаФорма.Номенклатура,ЭтаФорма.Серия.Номер ,ТекДок.Дата ) Тогда
...
Процедура СканЯчейкиИзменениеТекстаРедактирования(Элемент, Текст, СтандартнаяОбработка)
	Если СканЯчейкиИзменениеТекстаРедактированияНаСервереАдресОткуда(Текст) <> Ложь Тогда  
		АктивироватьКоличество();
	Иначе
		ОткрытьФормуСообщения("Ячейка не найдена");	
	КонецЕсли;
КонецПроцедуры  
...
Функция СканЯчейкиИзменениеТекстаРедактированияНаСервереАдресОткуда(ШтрихКод)   
	ЭтаФорма.АдресОткудаЗнач =  Справочники.СХК_СкладЯчейки.ПустаяСсылка();
	...
	Результат = Запрос.Выполнить().Выбрать();
	Если Результат.Следующий() Тогда
		АдресОткудаЗнач = Результат.Ссылка; 
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
Процедура АктивироватьПоискЯчейкиУп() 
	ОчиститьШКСервер(); 
	Элементы.АдресОткуда.ОбновитьТекстРедактирования();
	Элементы.АдресОткуда.Видимость = Ложь;
	Элементы.АдресОткуда.Видимость = Истина;
	ЭтаФорма.ТекущийЭлемент = Элементы.АдресОткуда; 
	# Если МобильныйКлиент Тогда
		НачатьРедактированиеЭлемента();
	#КонецЕсли
...
Процедура Разместить(Команда)
	...
	Если О = Ложь Тогда
		ЭтаФорма.Закрыть(); 
	Иначе
			Оповещение = Новый ОписаниеОповещения("ОбработатьЗакрытиеФормыСообщения", ЭтотОбъект);
...
Функция РазместитьНаСервере() 
	...
	Если Рез.Итог("ВНаличииОстаток")>=РасКол Тогда
		...
		Иначе
			...
			ТекДок.Автор =Пользователи.АвторизованныйПользователь(); 
		КонецЕсли;
		НоваяСтрока = ТекДок.ТЧТовары.Добавить();
		НоваяСтрока.Номенклатура = ЭтаФорма.Номенклатура;
		НоваяСтрока.Серия = ЭтаФорма.Серия;

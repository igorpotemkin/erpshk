...
Процедура ОткрытьФормуВыбор(ИмяФормы,СтруктураПараметры)
	...
	ОткрытьФорму("Обработка.СХК_МобильныйСклад.Форма."+ИмяФормы, _Параметры,ЭтаФорма,ЭтаФорма.УникальныйИдентификатор, , , ОписаниеОповещения,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
&НаСервере
Функция ПроверитьТайминг() 
	Возврат Обработки.СХК_МобильныйСклад.ПроверитьТайминг(ЭтаФорма.Сотрудник); 	
КонецФункции

&НаКлиенте
Процедура ВремяБлокировки() 
	
	Если Не ПроверитьТайминг() Тогда 
		ЭтаФорма.Закрыть();
	КонецЕсли;
	...
	//Окна = ПолучитьОкна(); 
	//
	//Для Каждого стр из Окна Цикл 
	//	Если Не стр.Основное Тогда
	//		Окна[стр].Закрыть();
	//	конецЕсли;
	//КонецЦикла;
	
	//	ПараметрыФормыОстатки = Новый Структура;
	//	ПараметрыФормыОстатки.Вставить("Пользователь",ЭтаФорма.Пользователь );
	//	АдресПередаваемыхПараметров = ПоместитьВоВременноеХранилище(ПараметрыФормыОстатки,Новый УникальныйИдентификатор);
	//
	//формаОбр = ПолучитьФорму("Обработка.СХК_МобильныйСклад.Форма.Форма", новый Структура("Адрес", АдресПередаваемыхПараметров));

	//Если формаОбр.Открыта() Тогда 
	//    формаОбр.Закрыть();
	//КонецЕсли;	

	//формадок = ПолучитьФорму("Обработка.СХК_МобильныйСклад.Форма.ФормаАвторизации");

	//формадок.Открыть();	
	
	//Если Не ПроверитьАктивность() Тогда  
		//ЭтаФорма.Закрыть();
		//ПараметрыФормыОстатки = Новый Структура;
		//АдресПередаваемыхПараметров = ПоместитьВоВременноеХранилище(ПараметрыФормыОстатки,Новый УникальныйИдентификатор);
		//_Параметры = новый Структура("Адрес", АдресПередаваемыхПараметров);
		//ОписаниеОповещения = Новый ОписаниеОповещения("ОбработатьЗакрытиеФормыАвторизация", ЭтаФорма);
		//ОткрытьФорму("Обработка.СХК_МобильныйСклад.Форма.ФормаАвторизации", _Параметры,ЭтаФорма, , , , ОписаниеОповещения,РежимОткрытияОкнаФормы.Независимый);
	//КонецЕсли;
...
&НаКлиенте
Процедура ОбработатьЗакрытиеФормыАвторизация(Значение, ДопПараметры) Экспорт 
					
КонецПроцедуры

&НаСервере
Функция ПроверитьАктивность()
	Возврат Истина;
КонецФункции
...
Процедура ОткрытьПоступление(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ОбработатьЗакрытиеФормыНовыйЗаказ", ЭтаФорма);
	ОткрытьФорму("Обработка.СХК_МобильныйСклад.Форма.ФормаПоступления", _Параметры,ЭтаФорма, , , , ОписаниеОповещения,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура Размещение(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ОбработатьЗакрытиеФормыНовыйЗаказ", ЭтаФорма);
	ОткрытьФорму("Обработка.СХК_МобильныйСклад.Форма.ФормаПеремещение", _Параметры,ЭтаФорма, , , , ОписаниеОповещения,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);

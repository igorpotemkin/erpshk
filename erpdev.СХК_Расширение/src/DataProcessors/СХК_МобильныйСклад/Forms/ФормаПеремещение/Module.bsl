...
Функция ПереместитьНаСервере() 
	...
	Иначе
		...
		КонецЕсли;
		Если ЭтаФорма.Количество>0 И ЭтаФорма.Количество <= СХК_ТСДСклад.ОстатокЯчейкаНоменклатура(ЭтаФорма.АдресОткудаЗнач,ЭтаФорма.Номенклатура,ЭтаФорма.Серия.Номер ,ТекДок.Дата ) Тогда

...
Процедура ПоискЗадания(Текст)
	...
	ЭтаФорма.Элементы.ИнформацияСклад.Заголовок ="";
	Если ТабЗнач.Количество()=1 Тогда 
		...
		//	ЭтаФорма.Элементы.Ошибка.Заголовок ="Склад получатель не верен";Элементы.СканЯчейки
		//Иначе
			...
			Если ТабЗнач[0].Статус = Перечисления.СХК_СтатусВыпуска.Собран Тогда
				...
				ЭтаФорма.Элементы.ИнформацияСклад.Заголовок ="Склад получатель :" + Строка(ТабЗнач[0].СкладПолучатель);
...
Функция ПринятьНаСервере()  
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	СХК_ВыпускПоЭтапу.ДопКоличество КАК ДопКоличество,
	               |	СХК_ВыпускПоЭтапу.ДопУпаковка КАК ДопУпаковка,
	               |	СХК_ВыпускПоЭтапу.ДопВыпуск КАК ДопВыпуск
	               |ИЗ
	               |	РегистрСведений.СХК_ВыпускПоЭтапу КАК СХК_ВыпускПоЭтапу
	               |ГДЕ
	               |	СХК_ВыпускПоЭтапу.ЭтапПроизводства = &ЭтапПроизводства
	...
	Если  Записи.Следующий() Тогда
		...
		ДопКоличество = Записи.ДопКоличество;		
		ДопУпаковка = Записи.ДопУпаковка;		
		ДопВыпуск = Записи.ДопВыпуск;		
	КонецЕсли;
	...
	//Если СкладПолучатель = Справочники.Склады.НайтиПоНаименованию("ГП Склад готовой продукции") Тогда 
		...
		Док.Ответственный =  ЭтаФорма.Сотрудник;
		...
		СтруктураПараметров.Вставить("Ответственный",ЭтаФорма.Сотрудник);
		...
		Если РасхОрдер.ОформленныеОрдера.Количество() > 0 Тогда
			...
			РОрдер.Ответственный = ЭтаФорма.Сотрудник;
		...
		РеквизитыШапки.Вставить("Автор",ЭтаФорма.Сотрудник);
		РеквизитыШапки.Вставить("Ответственный",ЭтаФорма.Сотрудник);
		...
		ДокПТ.Ответственный = ЭтаФорма.Сотрудник;
		Попытка 
		...
		Если ош = Ложь Тогда
			...
			НоваяЗапись.ДопКоличество =ДопКоличество;
			НоваяЗапись.ДопУпаковка =ДопУпаковка;
			НоваяЗапись.ДопВыпуск =ДопВыпуск;
		...
		КонецЕсли; 
	//Иначе
	// 	ЭтаФорма.Элементы.Ошибка.Заголовок = "Склад получатель не верен";
	//	ош = Истина	;
	//КонецЕсли;
...
Процедура СканЯчейкиИзменениеТекстаРедактированияНаСервере(ШтрихКод)   
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	И СХК_СкладЯчейки.Блокировка = Ложь";   
	...
	Иначе
		ЭтаФорма.Элементы.Ошибка.Заголовок = "Ячейка не найдена или заблокирована";
	КонецЕсли;
...
Процедура Группа1ПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	ИначеЕсли ТекущаяСтраница.Имя = "ГруппаСБ" Тогда 
		ОчиститьПоискСБ();
		АктивироватьПоискСБ();
...
Функция ПринятьРазместитьНаСервере() 
	...
	Иначе
		Если Рез.Итог("ВНаличииОстаток") = 0 Тогда
			...
			РезПоступлениеСклад = Запрос.Выполнить().Выбрать();
			
			Если РезПоступлениеСклад.Следующий() Тогда
				ТекДок = РезПоступлениеСклад.Ссылка.ПолучитьОбъект();
			Иначе
				ТекДок = Документы.Расш1_ПоступлениеСклад.СоздатьДокумент(); 
				ТекДок.Дата =ТекущаяДата(); 
		...
		Иначе
			...
			РезПоступление = Запрос.Выполнить().Выбрать();
			
			Если РезПоступление.Следующий() Тогда
				ТекДок = РезПоступление.Ссылка.ПолучитьОбъект();
			Иначе
				ТекДок = Документы.Расш1_ПоступлениеСклад.СоздатьДокумент(); 
				ТекДок.Дата =ТекущаяДата(); 
				ТекДок.Склад = Справочники.Склады.НайтиПоНаименованию("ГП Склад готовой продукции");
...
Функция ПринятьНаСервереПоОрдеру()  
	...
	Если ЭтаФорма.КоличествоОрдер*Числитель > 20000 Тогда
		ЭтаФорма.Элементы.ОшибкаОрдера.Заголовок = "Количество превышает 20000 шт"; 
		Возврат Истина; 
	Иначе
		ЭтаФорма.Элементы.ОшибкаОрдера.Заголовок = ""; 
	КонецЕсли;
...
/////////////////////////////////////////////////////////////////////////////////////////////// СБ  
&НаКлиенте
Процедура АктивироватьПоискСБ() Экспорт 

	ЭтаФорма.Элементы.СканПОрдераСБ.РедактированиеТекста = Истина;
 
	Элементы.СканПОрдераСБ.ОбновитьТекстРедактирования();
	Элементы.СканПОрдераСБ.Видимость = Ложь;
	Элементы.СканПОрдераСБ.Видимость = Истина;
	ЭтаФорма.ТекущийЭлемент = Элементы.СканПОрдераСБ; 
	# Если МобильныйКлиент Тогда
		НачатьРедактированиеЭлемента();
	#КонецЕсли
...
&НаСервере
Процедура ОчиститьПоискСБ() 

	ЭтаФорма.Элементы.ИнфоОрдераСБ.Заголовок = ""; 
	ЭтаФорма.Элементы.ГруппаКнопкиСБ.Видимость = Ложь;		
	ЭтаФорма.СканПОрдераСБ = "";
	ЭтаФорма.ДокРО = "";
КонецПроцедуры
...
&НаСервере
Функция ПроверитьСтатусПОрдераСБ(УИД) 
	СБ = Справочники.Склады.НайтиПоНаименованию("СБ Склад (Изолятор брака)");
	Попытка
	...
	Если Док = Неопределено ИЛИ Док.ПолучитьОбъект() = Неопределено Тогда 
		ош = "Отсканируйте правильный qr код";
	Иначе 
		Если Док.Склад = СБ И Док.Статус = Перечисления.СтатусыРасходныхОрдеров.КОтгрузке Тогда
			ЭтаФорма.ДокРО = Док.Ссылка;  
			ТабТоварыСБ.Загрузить(Док.ОтгружаемыеТовары.Выгрузить());
			ош = Истина;  
		ИначеЕсли Док.Склад <> СБ Тогда 
			ош = "Не верный склад";
			
		ИначеЕсли Док.Статус = Перечисления.СтатусыПриходныхОрдеров.Принят Тогда 
			ош = "Ордер уже принят";
		Иначе
			ош = "Измените статус ордера или склад";
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ПринятОрдерСБ(Команда)
	О = ПринятьНаСервереПоОрдеруБС();
	Если О = Ложь Тогда
		ЭтаФорма.Закрыть(); 
	Иначе
...
&НаСервере
Функция ПринятьНаСервереПоОрдеруБС()  
	
	ош = Ложь;  
	ДОрдер = ЭтаФорма.ДокРО.ПолучитьОбъект();
	ДОрдер.Статус = Перечисления.СтатусыРасходныхОрдеров.Отгружен;
	ДОрдер.Записать(РежимЗаписиДокумента.Проведение);   
	ЭтаФорма.ДокРО = ДОрдер.Ссылка;
//////////////////////////////     

	ТабРаспоряжений = ЭтаФорма.ДокРО.ТоварыПоРаспоряжениям.Выгрузить();
	...
	Для Каждого стр из ТабРаспоряжений Цикл
		...
		СтруктураП.Вставить("КлючевыеПоля","Номенклатура, Характеристика, Серия, Назначение, НазначениеОтправителя"); 
		МассивЗаказов = Новый Массив;
		МассивЗаказов.Добавить(стр.Распоряжение);
		СтруктураП.Вставить("МассивЗаказов",МассивЗаказов);
		
		РеквизитыШапки = Новый Структура; 
		РеквизитыШапки.Вставить("ВариантПриемкиТоваров",Перечисления.ВариантыПриемкиТоваров.РазделенаПоЗаказамИНакладным);
		РеквизитыШапки.Вставить("Организация",стр.Распоряжение.Организация);
		РеквизитыШапки.Вставить("СкладОтправитель",стр.Распоряжение.СкладОтправитель);
		РеквизитыШапки.Вставить("СкладПолучатель",стр.Распоряжение.СкладПолучатель);//Справочники.Склады.НайтиПоНаименованию("ГП Склад готовой продукции"));
		РеквизитыШапки.Вставить("СпособДоставки",Перечисления.СпособыДоставки.СиламиПеревозчикаДоНашегоСклада);
		РеквизитыШапки.Вставить("ХозяйственнаяОперация",Перечисления.ХозяйственныеОперации.ПеремещениеТоваров);
		РеквизитыШапки.Вставить("Автор",ЭтаФорма.Сотрудник);
		РеквизитыШапки.Вставить("Ответственный",ЭтаФорма.Сотрудник);
		...
		КонецПопытки;  
			...
			ДокПР = Документы.ПриходныйОрдерНаТовары.СоздатьДокумент();
			ДокПР.Дата = ТекущаяДата();
			ДокПР.Склад = стр.Распоряжение.СкладПолучатель;//Справочники.Склады.НайтиПоНаименованию("ГП Склад готовой продукции") ; 
			
			
			ДокПР.Распоряжение = стр.Распоряжение; 
			ДокПР.Статус = Перечисления.СтатусыПриходныхОрдеров.Принят;
			ДокПР.ДатаВходящегоДокумента = ТекущаяДата();
			ДокПР.Отправитель = стр.Распоряжение.СкладОтправитель ;
			ДокПР.Ответственный = Пользователи.АвторизованныйПользователь();
			ДокПР.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеремещениеТоваров;
			ДокПР.СкладскаяОперация = Перечисления.СкладскиеОперации.ПриемкаПоПеремещению;
			ДокПР.Товары.Загрузить(ДОрдер.ОтгружаемыеТовары.Выгрузить()); 
		Попытка 
			ДокПР.Записать(РежимЗаписиДокумента.Проведение);  
		Исключение 
			ДокПР.Записать(РежимЗаписиДокумента.Запись); 
		...
		ДокПРСсылка = ДокПР.Ссылка;

		Если ДокПРСсылка.Проведен Тогда
			...
			Иначе
				ТекДок = Документы.Расш1_ПоступлениеСклад.СоздатьДокумент(); 
				ТекДок.Дата =ТекущаяДата(); 
				ТекДок.Склад = стр.Распоряжение.СкладПолучатель;//Справочники.Склады.НайтиПоНаименованию("ГП Склад готовой продукции");
			КонецЕсли; 
			табТоваровП = ДокПРСсылка.Товары.Выгрузить();
			Для каждого тСтр из табТоваровП Цикл
				НоваяСтрока = ТекДок.Товары.Добавить();
				НоваяСтрока.Номенклатура = тСтр.Номенклатура;
				НоваяСтрока.Серия = тСтр.Серия.Номер;
				НоваяСтрока.Количество = тСтр.КоличествоУпаковок;
				Если ЗначениеЗаполнено(тСтр.Серия.СХК_Упаковка) Тогда
					НоваяСтрока.Упаковка = тСтр.Серия.СХК_Упаковка;
				Иначе
					НоваяСтрока.Упаковка = тСтр.Номенклатура.ЕдиницаИзмерения;
				КонецЕсли;					
				НоваяСтрока.Время = ТекущаяДата(); 
				НоваяСтрока.Ячейка = Справочники.СХК_СкладЯчейки.НайтиПоНаименованию("Буфер");;
				НоваяСтрока.Пользователь = ЭтаФорма.Сотрудник;

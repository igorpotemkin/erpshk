...
Процедура ЗаполнитьУпаковкаУп(Штр) 
	//Уп = СХК_ТСДСклад.ЗаполнитьУпаковкаУп(Штр,ЭтаФорма.Номенклатура);
	//Если Уп.Пустая() Тогда 
	//	ЭтаФорма.УпаковкаУп = ЭтаФорма.Номенклатура.ЕдиницаИзмерения;	
	//Иначе
...
Процедура Разместить(Команда)
	...
	Если О.Ош = Ложь Тогда //И О.РасКолОстаток = 0 Тогда
		ЭтаФорма.Закрыть(); 
	...
	Если РасКол > 0 Тогда
		...
		Иначе
			...
			Запрос.УстановитьПараметр("Автор", Пользователи.АвторизованныйПользователь()); 
				...
				///////////	Добавляем остаток
				Если РасКол - Рез.Итог("ВНаличииОстаток") > 0 Тогда
					НоваяСтрока = ТекДок.ТЧТовары.Добавить();
					НоваяСтрока.Номенклатура = ЭтаФорма.Номенклатура;
					НоваяСтрока.Серия = ЭтаФорма.Серия;
					НоваяСтрока.ТекЯчейка = Буфер; 

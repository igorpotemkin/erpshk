...
СтартТайминг() ;	
	Если СокрЛП(Текст) = "0000003720000" ИЛИ СокрЛП(Текст) = "0000011111110" Тогда
		ОткрытьФормуСообщения("Из буфера и Спайки отгрузка запрещена");
	Иначе
		Ош = ВыборЯчейки(Текст);
...
Функция ДобавитьВСборку(Номен,Сер,Штрих,Яч,Кол)
	...
	Если КолДок >= КолСобрано И СтрокиТПР[0].Собран = Ложь Тогда 
		...
		ТабСборки.Ячейка = ЯчейкаВыбор;
		ТабСборки.ШКУпаковки = ШКУпаковки; 
		ТабСборки.ДатаВремя = ТекущаяДата();
		ТабСборки.Кладовщик = ЭтаФорма.Сотрудник;

...
Процедура ЗаполнитьТабЧасть()
	...
	Для Каждого стр из Товары Цикл
		Если стр.КоличествоУпаковок<=0 ИЛИ стр.Номенклатура.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Услуга Тогда
			Товары.Удалить(стр);
		КонецЕсли;
	КонецЦикла;
	
	Если Товары.Количество()<6 Тогда
...
|	СУММА(ПриобретениеТоваровУслугТовары.КоличествоУпаковок) КАК КоличествоПринято
|ПОМЕСТИТЬ ВТЗПоступление
|ИЗ
...
|	Выбор Когда ВТЗ.Номенклатура = ВТЗПоступление.Номенклатура Тогда ВТЗ.КоличествоУпаковок - ВТЗПоступление.КоличествоПринято Иначе ВТЗ.КоличествоУпаковок Конец  КАК КоличествоУпаковок
|
|ИЗ
|	ВТЗ КАК ВТЗ
|		ЛЕВОЕ СОЕДИНЕНИЕ ВТЗПоступление КАК ВТЗПоступление
|		ПО ВТЗ.Номенклатура = ВТЗПоступление.Номенклатура";
...
Процедура Принять(Команда)
	...
	СтруктураСтрока = ЗаполнитьСтруктуруОтбора();
	
	Если СтруктураСтрока.Количество()>=1 Тогда 
		ЭтаФорма.Закрыть();
		ПараметрыФормыОстатки = Новый Структура;
		ПараметрыФормыОстатки.Вставить("ЗаказПоставщику",ЭтотОбъект.ДокументЗаказПоставщику);
		
		ПараметрыФормыОстатки.Вставить("СтрокиЗаказПоставщику",СтруктураСтрока);
		...
		ОткрытьФорму("Обработка.СХК_МобильныйСкладСырье.Форма.ФормаПоступления", _Параметры,ЭтаФорма, , , , ОписаниеОповещения,РежимОткрытияОкнаФормы.Независимый);
	Иначе
		Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияПредупреждение", ЭтотОбъект);	
		ПоказатьПредупреждение(Оповещение,"Выберите строки",5,"Внимание!!!");
	КонецЕсли;
...
Функция ЗаполнитьСтруктуруОтбора() 
	...
	Если Строки.Количество() >=1 Тогда
		Для сч=0 По Строки.Количество()-1 Цикл
			СЗ = Новый Структура;
			СЗ.Вставить("Номенклатура",Строки[сч].Номенклатура);
			СЗ.Вставить("КоличествоУпаковок",Строки[сч].КоличествоУпаковок);
			СЗ.Вставить("Упаковка",Строки[сч].Упаковка);
			Мас.Добавить(СЗ);
		КонецЦикла;
	КонецЕсли;
...
Процедура ТоварыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
		СтандартнаяОбработка = Ложь;  
		Если Элемент.ТекущиеДанные.Выбор = Ложь Тогда
			Элемент.ТекущиеДанные.Выбор = Истина;
		Иначе
			Элемент.ТекущиеДанные.Выбор = Ложь;
		КонецЕсли;
...
//	
//	Для Каждого стр из Товары Цикл 
//		Если стр.КоличествоУпаковокПринято >= стр.КоличествоУпаковок Тогда
//			Товары.Удалить(стр); 
//		Иначе
//			стр.КоличествоУпаковок = стр.КоличествоУпаковок - стр.КоличествоУпаковокПринято;
//		КонецЕсли;
//	КонецЦикла;
//	
//КонецПроцедуры
...
//			
//			Если Строки.Количество()>=1 Тогда 
//				Если стр.КоличествоУпаковокПринято > 0 Тогда

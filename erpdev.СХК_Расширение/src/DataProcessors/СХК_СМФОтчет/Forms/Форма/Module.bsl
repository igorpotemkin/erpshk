...
Процедура СформироватьП2(Команда)
	...
	Функция ПечатьНаСервереПриложение2()  
		...
		Подвал.Параметры.СуммаКВыплате = ?((ЭтаФорма.МинимальныйОстаток-ИтСуммаРоялти)>0,ЭтаФорма.МинимальныйОстаток-ИтСуммаРоялти,0);
		Подвал.Параметры.СуммаОстатка = ?((ЭтаФорма.МинимальныйОстаток-ИтСуммаРоялти)>0,ЭтаФорма.МинимальныйОстаток-ИтСуммаРоялти,0);
		...
		Для Каждого стрК из ТабК Цикл 
			...
			КАСтрока.Параметры.Контрагент		 =стрК.Получатель;	

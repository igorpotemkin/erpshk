...
Функция	ЗаполнитьТЧ(НомП)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	РегистрНакопления.СХК_ТоварыНаСкладе.Остатки КАК СХК_ТоварыНаСкладеОстатки
	               |ГДЕ
	               |	СХК_ТоварыНаСкладеОстатки.Ячейка.ПометкаУдаления = &ПометкаУдаления И СХК_ТоварыНаСкладеОстатки.Ячейка.Блокировка = Ложь"; 
...
////////////////////// 
	...
	ВидНоменклатуры.Добавить(Справочники.ВидыНоменклатуры.НайтиПоНаименованию("Готовая продукция: Батончики"));
	ВидНоменклатуры.Добавить(Справочники.ВидыНоменклатуры.НайтиПоНаименованию("Готовая продукция: Готовые завтраки"));
	ВидНоменклатуры.Добавить(Справочники.ВидыНоменклатуры.НайтиПоНаименованию("Готовая продукция: Наборы")); 
	
	ЗапросГП = Новый Запрос;
	ЗапросГП.Текст = "ВЫБРАТЬ
|	ТоварыНаСкладахОстатки.ВНаличииОстаток КАК ВНаличииОстаток
|ИЗ
|	РегистрНакопления.ТоварыНаСкладах.Остатки(&ДатаЗапроса,Склад = &Склад И Номенклатура.ВидНоменклатуры В (&ВидНоменклатуры) ) КАК ТоварыНаСкладахОстатки
|"; 
	ЗапросГП.УстановитьПараметр("ДатаЗапроса",ТекущаяДата());
	ЗапросГП.УстановитьПараметр("ВидНоменклатуры",ВидНоменклатуры);
	ЗапросГП.УстановитьПараметр("Склад",Справочники.Склады.НайтиПоНаименованию("ГП Склад готовой продукции"));
	РезГП = ЗапросГП.Выполнить().Выбрать();
	
	Если РезГП.Следующий() Тогда
		ОстатокГП = РезГП.ВНаличииОстаток;
	Иначе                                 
		ОстатокГП = 0 ;
	КонецЕсли;
	
//////////////////////////////ЦК Цех Комплектации
	ЗапросЦК = Новый Запрос;
	ЗапросЦК.Текст = "ВЫБРАТЬ
|	ТоварыНаСкладахОстатки.ВНаличииОстаток КАК ВНаличииОстаток
|ИЗ
|	РегистрНакопления.ТоварыНаСкладах.Остатки(&ДатаЗапроса,Склад = &Склад И Номенклатура.ВидНоменклатуры В (&ВидНоменклатуры) ) КАК ТоварыНаСкладахОстатки
|"; 
	ЗапросЦК.УстановитьПараметр("ДатаЗапроса",ТекущаяДата());
	ЗапросЦК.УстановитьПараметр("ВидНоменклатуры",ВидНоменклатуры);
	ЗапросЦК.УстановитьПараметр("Склад",Справочники.Склады.НайтиПоНаименованию("ЦК Цех Комплектации"));
	РезЦК = ЗапросЦК.Выполнить().Выбрать();
	
	Если РезЦК.Следующий() Тогда
		ОстатокЦК = РезЦК.ВНаличииОстаток;
	Иначе                                 
		ОстатокЦК = 0 ;
	КонецЕсли;
//////////////////////////////////////
...
//
		...
		Ряд9.Параметры.ОстатокГП = ОстатокГП;
		Ряд9.Параметры.ОстатокЦК = ОстатокЦК;

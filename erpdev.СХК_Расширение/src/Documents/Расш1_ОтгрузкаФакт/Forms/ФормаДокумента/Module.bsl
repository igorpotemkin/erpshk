...
Функция ПечатьНаСервере()
	ТаблДокум = Новый  ТабличныйДокумент;
	ТаблДокум.Очистить();
	Макет = РеквизитФормыВЗначение("Объект").ПолучитьМакет("ОтгрузочныйЛист");  
	Шапка = Макет.ПолучитьОбласть("Шапка");
	Шапка.Параметры.ДатаОтгрузки	= "Дата отгрузки: " + Строка(Формат(Объект.Дата,"ДЛФ=Д"));
	Шапка.Параметры.Транспорт		= "Транспорт: " + Строка(Объект.Транспорт);
	Шапка.Параметры.Водитель	= "Водитель: " + Строка(Объект.ФИОВодителя);
	Шапка.Параметры.Паллет	= "Паллет мест: " + Строка(Объект.ПалетМест);
	...
	Для Каждого стр из Объект.ТЧОрдер Цикл
		...
		Строка.Параметры.Контрагент = стр.ЗаказКлиента.Контрагент;
		Строка.Параметры.Адрес = стр.ЗаказКлиента.АдресДоставки;
		Строка.Параметры.НомерРО = РООбъем(стр.ЗаказКлиента).РО; 
		ГК = РООбъем(стр.ЗаказКлиента).ГК;
		ИтогКолКоробов = ИтогКолКоробов +ГК;
		Строка.Параметры.КолКоробов = ГК;
		
		ШБ = РООбъем(стр.ЗаказКлиента).ШБ;
		ИтогКолШоу = ИтогКолШоу +ШБ;
		Строка.Параметры.КолШоу = ШБ;
	...
	Итоги.Параметры.ИтогКолКоробов = ИтогКолКоробов;
	Итоги.Параметры.ИтогКолШоу = ИтогКолШоу;
	
	ТаблДокум.Вывести(Итоги); 
...
Функция РООбъем(Ссылка)
		...
		Для Каждого строка из Рез Цикл
			Если ТипЗнч(строка.Ссылка) = ТИП("ДокументСсылка.РасходныйОрдерНаТовары") Тогда  
				...
				Если строка.Ссылка.ПометкаУдаления = Ложь И строка.Ссылка.Проведен = Истина Тогда
                	РО = РО + Строка(строка.Ссылка.Номер) + Символы.ПС;	
					
					Для Каждого стр из строка.Ссылка.ОтгружаемыеТовары Цикл
						Если СтрНайти(стр.Упаковка.Типоразмер.Наименование, "гофро") 
							ИЛИ  СтрНайти(стр.Упаковка.Типоразмер.Наименование, "г/к")	
							ИЛИ  СтрНайти(стр.Упаковка.Типоразмер.Наименование, "Г/К") Тогда	
								ГК = ГК + стр.КоличествоУпаковок;
							
							ИначеЕсли СтрНайти(стр.Упаковка.Типоразмер.Наименование, "шоу") Тогда 
								ШБ = ШБ + стр.КоличествоУпаковок;
						КонецЕсли;		
					КонецЦикла;		
				КонецЕсли;
			КонецЕсли;
